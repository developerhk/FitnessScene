...
+ (MBProgressHUD *)showHUDForFitnessAddedTo:(UIView *)view  {
    MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:view HK:YES];
    [view addSubview:hud];
    [hud show:YES];
...
- (id)initWithView:(UIView *)view HK:(BOOL)bol{
...
- (void)setupLabels {
	...
	detailsLabel.textAlignment = TextAlignmentCenter;
...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
        ...
        UIActivityIndicatorView *vv = [[UIActivityIndicatorView alloc]
                                       initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge];
        #pragma mark - HK
        if(HKADD)
        {
            vv.color = UIColorFromRGB(0x4e4d4d);
            vv.backgroundColor = [UIColor clearColor];
        }
        #pragma mark - HK
		self.indicator = MB_AUTORELEASE(vv);
