...
@interface CounselViewController ()<StarListDelegate,BlackGroundViewDelegate,UIAlertViewDelegate>
{
    CounselModel *_dataModel;
...
- (void)viewDidLoad {
    ...
    [self setNavTitleWithKaiti:@"心理辅导"];
...
-(void)requestForStarZone
{
    ...
    NSString *path = [NSString stringWithFormat:@"%@?userID=%@", Request_GetCoachVideoInfo,userID];
...
- (void)starZoneSuccess:(id)response
{
    ...
    if ([[response objectForKey:@"Result"] intValue] == 1)
    {
        ...
        {
            _dataModel = nil;
            _dataModel = [CounselModel CMWithInfo:dic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.row == 0)
    {
        ...
        CounselHeaderCell *cell = (CounselHeaderCell *)[tableView dequeueReusableCellWithIdentifier:identifier];
        if (cell == nil) {
            cell = [[NSBundle mainBundle] loadNibNamed:@"CounselHeaderCell" owner:self options:nil][0];
        }
...
-(void)starCell:(StarListCell *)cell withtag:(int)tag isLock:(BOOL)isLock
{
    ...
    [MobClick event:ProfessorCourse_PlayCounselVideo attributes:@{@"VID" : model.topicName}];
    
    if(isLock)
    {
        index = tag;
        _episode = model.episode;
        _unlockType = @"CoachLock";
        [self requestForTask];
        return;
    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.row == 0)
    {
        return 230 + [FMUString heightForText:_dataModel.videoContent withTextWidth:self.view.bounds.size.width - 50  withFont:[UIFont systemFontOfSize:14]];;
    }
    else
    {
        return 186.0f;
